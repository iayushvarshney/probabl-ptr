...
// Reachability check — some webhook UIs probe the URL with a plain
// GET/HEAD before saving it. Always answer 200 here; no signal processing
// happens on this path, so it doesn't weaken the POST auth check.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // than a real event. Authentication already succeeded, so acknowledge
    // receipt instead of erroring.
    console.warn("[posthog webhook] empty or non-JSON body — treating as a connectivity check");
  ...
  if (!signal) {
    ...
    // rather than a real event. Acknowledge receipt (2xx) so webhook
    // "test connection" UIs don't read this as broken; still logged for
    // visibility into genuinely malformed real payloads.
    console.warn("[posthog webhook] authenticated request had no identifiable person — not stored", rawPayload);
    return NextResponse.json({ ok: true, received: true, stored: false });
  }
