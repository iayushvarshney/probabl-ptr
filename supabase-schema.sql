...
-- ─────────────────────────────────────────────────────────────
-- companies: Reo.dev account-enrichment fields. customer_fit and
-- activity_score* are persisted only (not yet wired into scoring — see
-- TODO in src/lib/rollup.ts); industry, employee_count_range,
-- preferred_technology, and country feed the ICP evaluation in
-- src/lib/icp.ts. Safe to re-run (idempotent).
-- ─────────────────────────────────────────────────────────────
alter table companies add column if not exists customer_fit text;
...
-- ─────────────────────────────────────────────────────────────
-- settings: versioned scoring/ICP configuration. Append-only — saving
-- never overwrites a row, it inserts the next config_version for that
-- section, so history is preserved and "reset to defaults" is just another
-- version. The "current" config for a section is the row with the highest
-- config_version.
-- ─────────────────────────────────────────────────────────────
create table if not exists settings (
  id             uuid primary key default gen_random_uuid(),
  section        text not null,              -- 'scoring' | 'icp'
...
-- tag/filter by it (e.g. "Customer") without a live HubSpot call per
-- entity on every queue load. is_customer is a derived convenience
-- boolean (true when hubspot_lifecycle_stage = 'customer'), recomputed
-- alongside it so callers never have to string-compare. The entity
-- detail page still fetches lifecycle stage live and only falls back to
-- this stored value if that live call fails.
-- ─────────────────────────────────────────────────────────────
